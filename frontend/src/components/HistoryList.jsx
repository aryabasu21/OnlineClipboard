...
export function HistoryList({
  ...
  (async () => {
    ...
    const Prism = (await import(/* webpackChunkName: 'prism-core' */ "prismjs")).default || (await import("prismjs"));
    ...
    <ul className="history-grid" role="list" aria-label="Clipboard history">
      {items
        ...
        .map((item) => {
          ...
          const showExpand = !!item.preview && (item.preview.length > 140 || item.preview.includes("\n"));
          return (
            <li
              key={item.version}
              className={
